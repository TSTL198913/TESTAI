...
async def test_real_network_with_assertions_passed():
    ...
    respx.get("https://httpbin.org/json").mock(
        return_value=httpx.Response(200, json={"slideshow": {"author": "Yours Truly"}})
    )

    context = ExecutionContext(
        case_id="real_assert_pass", env={"base_url": "https://httpbin.org"}
    )
    ...
    step = HttpRequest(
        ...
        assertions=[
            ...
            Assertion(
                check="jsonpath", path="$.slideshow.author", expected="Yours Truly"
            ),
        ],
    )
...
async def test_real_network_with_assertions_failed():
    ...
    context = ExecutionContext(
        case_id="real_assert_fail", env={"base_url": "https://httpbin.org"}
    )
    ...
    step = HttpRequest(
        ...
        method="GET",
        assertions=[Assertion(check="status_code", expected=500)],
    )
