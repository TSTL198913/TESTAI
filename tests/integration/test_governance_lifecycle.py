...
async def test_governance_lifecycle_triggered():
    ...
    pipeline = ExecutionPipeline(
        processors=[DispatchProcessor(), AssertionProcessor(), GovernanceProcessor()]
    )
    ...
    assert result is not None, "步骤结果不应为空"
    assert "governance_insight" in result, "治理引擎未生成治理报告 - GovernanceProcessor 可能未执行"
    assert result.get("status") == "FAILED", "步骤状态应为 FAILED"

    insight = result["governance_insight"]
    print(f"\n[Success] 治理报告已生成: {insight.get('reasoning')}")
    assert insight.get("reasoning") is not None, "治理报告应包含推理内容"
    assert "confidence_score" in insight, "治理报告应包含 confidence_score 字段"
