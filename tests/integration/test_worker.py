...
class TestWorkerTasks:
    ...
    def test_task_with_default_pipeline_config(self):
        request_dict = {"case_id": "default_config_test", "steps": []}
        # 验证默认配置包含 governance（NEW-004 修复后）
        assert request_dict.get(
            "pipeline", ["data", "request", "assertion", "governance"]
        ) == ["data", "request", "assertion", "governance"]
    ...
    def test_task_module_structure(self):
        ...
        # 行为验证：模块必须导出核心组件
        assert callable(
            getattr(tasks_module, "run_test_pipeline", None)
        ), "tasks_module 必须导出 run_test_pipeline 函数"
        assert tasks_module.celery_app is not None, "tasks_module 必须导出 celery_app"
        assert (
            tasks_module.AsyncLoopManager is not None
        ), "tasks_module 必须导出 AsyncLoopManager"
        assert (
            tasks_module.ResourceContainer is not None
    ...
    def test_task_empty_steps_validation(self):
        request_dict = {"case_id": "empty_steps_validation", "steps": []}
    ...
    def test_task_celery_delay_method(self):
        with patch.object(run_test_pipeline, "delay") as mock_delay:
            ...
            assert (
                getattr(task_result, "id", None) == "test-task-id"
            ), "delay 返回值必须有 id 属性"
