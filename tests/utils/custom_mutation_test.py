...
class CustomMutationTester:
    def __init__(self, target_dir: str = "src/governance/", test_command: str = "python -m pytest -x -q"):
    ...
    def _generate_mutations(self, file_path: str) -> List[Tuple[int, str, str]]:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith("return "):
            ...
            elif "<" in stripped and not stripped.startswith("#") and "def " not in stripped and "<=" not in stripped:
                mutations.append((line_num, stripped, stripped.replace("<", ">")))
                ...
                mutations.append((line_num, stripped, stripped.replace("while ", "while False and ")))
            
            elif "+" in stripped and not stripped.startswith("#") and "def " not in stripped and "=" not in stripped:
                mutations.append((line_num, stripped, stripped.replace("+", "-")))
            
            elif "-" in stripped and not stripped.startswith("#") and "def " not in stripped and "=" not in stripped and "return" not in stripped:
                mutations.append((line_num, stripped, stripped.replace("-", "+")))
            ...
            elif '"' in stripped and not stripped.startswith("#") and "return" in stripped:
                mutations.append((line_num, stripped, stripped.replace('"', "'")))
            
            elif "'" in stripped and not stripped.startswith("#") and "return" in stripped and '"' not in stripped:
                mutations.append((line_num, stripped, stripped.replace("'", '"')))
            
            elif "len(" in stripped and not stripped.startswith("#"):
                mutations.append((line_num, stripped, stripped.replace("len(", "len(")))
    ...
    def _restore_file(self, file_path: str, line_num: int, original_line: str):
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
    ...
    def _restore_from_backup(self, file_path: str):
    ...
    def run(self) -> dict:
        ...
        for file_path in py_files:
            ...
            for line_num, original, mutated in mutations:
                ...
                try:
                    self._apply_mutation(file_path, line_num, mutated)
                    
                    test_failed, error = self._run_tests()
    ...
    def _generate_report(self) -> dict:
        kill_rate = (self.mutations_killed / self.mutations_applied * 100) if self.mutations_applied > 0 else 0
