...
| 业务域 | 核心场景 | API端点数量 | 实现状态 | S级测试覆盖 |
|--------|----------|-------------|----------|-------------|
| **测试执行引擎** | HTTP/gRPC测试、测试用例生成、缺陷分析 | 1+ | ✅ 完整 | ✅ 2个 |
| **AI治理闭环** | 诊断→修复→审批→收敛验证 | 4 | ✅ 完整 | ✅ 10个 |
| **工作流自动化** | DAG任务编排、依赖管理、状态追踪 | 3 | ✅ 完整 | ✅ 3个 |
| **平台管理** | 用户管理、团队协作、配置管理、监控告警 | 20+ | ✅ 完整 | ✅ 6个 |
| **质量保障** | 变异测试、黄金数据集、基线验证 | - | ✅ 完整 | ✅ 3个 |
...
| 场景ID | 场景名称 | 描述 | 技术实现 | 测试覆盖 |
|--------|----------|------|----------|----------|
| DG-001 | AI深度诊断 | LLM分析错误上下文生成修复建议 | `AIGovernanceAgent.analyze_with_context()` | ✅ S级 |
...
| 场景ID | 场景名称 | 描述 | 技术实现 | 测试覆盖 |
|--------|----------|------|----------|----------|
| FX-001 | 函数级补丁应用 | 精确替换函数体 | `FunctionTransformer` + libcst | ✅ S级 |
| FX-002 | 类方法级补丁 | 精确替换类中的指定方法 | `ContextAwareTransformer` | ✅ S级 |
| FX-003 | 安全代码校验 | 检测危险函数调用(eval/exec) | `SecurityVisitor` | ✅ S级 |
| FX-004 | 路径安全校验 | 防止路径遍历攻击 | `SecurePathValidator` | ✅ S级 |
...
| 场景ID | 场景名称 | 描述 | 技术实现 | 测试覆盖 |
|--------|----------|------|----------|----------|
| AP-001 | 审批创建 | 创建补丁审批记录 | `ApprovalManager.create_approval()` | ✅ S级 |
| AP-002 | 自动审批判断 | 根据补丁类型决定是否需要审批 | `requires_approval`属性 | ✅ S级 |
| AP-003 | 人工审批 | 管理员审批补丁 | `ApprovalManager.approve()` | ✅ S级 |
| AP-004 | 审批拒绝 | 管理员拒绝补丁 | `ApprovalManager.reject()` | ✅ |
| AP-005 | 审批过期 | 30分钟自动过期 | `is_expired`属性 | ✅ |
| AP-006 | 审批记录持久化 | SQLite存储审批历史 | `_save_to_db()` | ✅ S级 |

### 3.4 收敛验证场景

| 场景ID | 场景名称 | 描述 | 技术实现 | 测试覆盖 |
|--------|----------|------|----------|----------|
| CV-001 | 基线验证 | 验证实际输出与基线的一致性 | `GoldenBaselineManager.validate_against_baseline()` | ✅ S级 |
| CV-002 | 收敛分数计算 | 量化验证结果 | `calculate_convergence_score()` | ✅ S级 |
| CV-003 | 变异测试 | 验证测试用例的有效性 | `CustomMutationTester` | ✅ S级 |
...
| 场景ID | 场景名称 | 描述 | 技术实现 | 测试覆盖 |
|--------|----------|------|----------|----------|
| WF-001 | 工作流定义 | 创建包含多个任务的工作流 | `WorkflowEngine.define_workflow()` | ✅ S级 |
| WF-002 | 任务依赖管理 | 定义任务之间的执行顺序 | `WorkflowTask.depends_on` | ✅ S级 |
| WF-003 | 拓扑排序执行 | 基于DAG的任务调度 | `_calculate_execution_order()` | ✅ S级 |
...
|--------|----------|------|----------|----------|
| AU-001 | 用户登录 | 用户名密码认证获取Token | `POST /auth/login` | ✅ S级 |
| AU-002 | Token刷新 | 使用Refresh Token获取新的Access Token | `POST /auth/refresh` | ✅ |
| AU-003 | 获取当前用户 | 获取登录用户信息 | `GET /auth/me` | ✅ |
| AU-004 | 权限校验 | 基于角色的权限控制 | `require_permission()`依赖 | ✅ S级 |

### 5.2 用户管理场景

| 场景ID | 场景名称 | 描述 | API端点 | 测试覆盖 |
|--------|----------|------|----------|----------|
| UM-001 | 创建用户 | 创建新用户账户 | `POST /users` | ✅ S级 |
...
| 场景ID | 场景名称 | 描述 | API端点 | 测试覆盖 |
|--------|----------|------|----------|----------|
| TM-001 | 创建团队 | 创建新团队 | `POST /teams` | ✅ S级 |
...
| 场景ID | 场景名称 | 描述 | API端点 | 测试覆盖 |
|--------|----------|------|----------|----------|
| CM-001 | 获取配置 | 获取指定section的配置 | `GET /config` | ✅ S级 |
| CM-002 | 更新配置 | 更新指定section的配置 | `PUT /config/{section}` | ✅ S级 |
...
| MO-002 | 获取告警列表 | 按级别筛选告警 | `GET /monitoring/alerts` | ✅ S级 |
| MO-003 | 确认告警 | 标记告警为已处理 | `POST /monitoring/alerts/{alert_id}/acknowledge` | ✅ S级 |
...
| governance/approval.py | 4 | 95% |
| governance/executor.py | 2 | 92% |
| governance/orchestrator.py | 1 | 100% |
| governance/tracker.py | 2 | 98% |
| governance/baseline.py | 2 | 89% |
| governance/security.py | 2 | 84% |
| governance/resilience.py | 2 | 100% |
| governance/file_lock.py | 1 | 94% |
| governance/git_manager.py | 2 | 72% |
| platform/api.py | 2 | 97% |
| platform/workflow.py | 3 | 91% |
| platform/config_manager.py | 2 | 78% |
| governance/monitoring.py | 2 | 95% |
