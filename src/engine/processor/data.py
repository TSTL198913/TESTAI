...
class DataProcessor(BaseProcessor):
    ...
    def _render_and_update(self, step, data_dict, field_name: str):
        lookup = self._get_lookup_dict(self.context_ref)  # 稍微改动下获取方式，或直接传参
        # 实际代码中可以直接通过参数传入 lookup_dict
        return render_template(data_dict, self._get_lookup_dict(self.context_ref), strict=self.strict)
    ...
    async def _process_http(self, step: HttpRequest, context: ExecutionContext) -> HttpRequest:
    ...
    async def _process_grpc(self, step: GrpcRequest, context: ExecutionContext) -> GrpcRequest:
